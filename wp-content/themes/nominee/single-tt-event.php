                        ...
                        <?php if (nominee_option('event-date-counting', false, true)): ?>
                            <div class="countdown-wrapper">
                                <?php if (function_exists('rwmb_meta')) :
                                    ...
                                    if ($event_date) : ?>
                                        <ul class="countdown list-inline" data-countdown="<?php echo esc_attr($event_date.' '.$event_end_time); ?> "></ul>
                                    <?php endif; 
                                endif; ?>
                            </div>
                        <?php endif ?>
                        ...
                        <?php if (nominee_option('event-map-visibility', false, true)): ?>
                            <?php if (function_exists('rwmb_meta')) :
                                $args = apply_filters('nominee_single_event_map', array(
                                    ...
                                    'marker'       => true,
                                    'zoom'         => 15,
                                    'marker_title' => rwmb_meta('event-location'),
                                    'info_window'  => rwmb_meta('event-location')
                                ));

                                rwmb_the_value('nominee_event_location_map', $args);
                            endif; ?>
                        <?php endif; ?>

                        <?php if (nominee_option('event-passed-notice', false, true)): ?>
                            <!-- Event passed notice -->
                            <?php if (function_exists('rwmb_meta')) :
                                ...
                                if ($current_date > $contest_tart) : ?>
                                    <div class="event-passed-message">
                                        <div class="alert-warning">
                                            <i class="fa fa-exclamation-triangle"></i><?php echo esc_html(nominee_option('event-passed-notice-text')); ?>
                                        </div>
                                    </div>
                                <?php endif;
                            endif; ?>

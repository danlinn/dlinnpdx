 ...
 * @since 1.0.0
 */
if (!class_exists('business_planning_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class business_planning_About
	{
		/**
		 ...
		 *
		 * @var business_planning_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof business_planning_About)) {
				self::$instance = new business_planning_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<a href="' . esc_url('https://wpthemespace.com/product/business-planning-pro/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://wpthemespace.com/product/business-planning-pro/') . '" target="_blank" >';
					echo esc_html('Business Planning Pro Live Preview', 'business-planning');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'business-planning')
					)
				);
			}
		}
